...
VcProjectBuildOptionsPage::VcProjectBuildOptionsPage() :
    m_optionsWidget(0)
{
    setId("VcProject.MSBuild");
    setDisplayName(tr("MS Build"));
    setCategory(VcProjectManager::Constants::VC_PROJECT_SETTINGS_CATEGORY);
...
void VcProjectBuildOptionsPage::versionCheckFinished()
{
    if (m_validator.m_process) {
        QString response = QString::fromUtf8(m_validator.m_process->readAll());
